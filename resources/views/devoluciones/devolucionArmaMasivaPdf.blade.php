...
<head>
    <meta charset="UTF-8">
    <title>ACTA DEVOLUCION MASIVA.pdf</title>
...
<header>
    <table>
        ...
        <tr>
            <th style="text-align: left; font-size:11px" width="320px">NRO. DE ACTA: {{ $devolucion->nro_nota }}</th>
        </tr>
    </table>
...
<body>
    <section>
        ...
        <p style="text-align: center"><b><u>ACTA DE DEVOLUCION MASIVA</u></b></p>

        <p style="text-align: justify">En la Ciudad de <b>{{$devolucion->localidadRecibe->nom_localidad}}</b>, Departamento
            <b>{{$devolucion->localidadRecibe->departamento->nom_departamento}}</b>, Provincia de Santa Fe, a los {{$dia}} días
            del mes de {{$nombre_mes_actual}} del año {{$anio}}, quién suscribe <b>NI: {{$devolucion->usuario->legajo}} -
                {{$devolucion->usuario->personal->nombre_ps}}</b>, logístico de el/la <b>{{$devolucion->usuario->unidad->nom_ud}}</b>, procede a labrar
            la presente a fin de dejar debidamente documentado la devolucion por parte de el/la <b>{{$devolucion->subUdAnterior->nom_subud}}</b> de el/la
            <b>{{$devolucion->UdAnterior->nom_ud}}</b>, la cantidad de <b>{{ '('.$armas->count().')' }}</b> arma/s que se detalla/n a continuación.
            <br>Se presenta por la misma el personal <b>NI: {{$devolucion->personal->nlegajo_ps}} - {{$devolucion->personal->nombre_ps}}</b>.
        </p>
        <br>
        <table style="border-collapse: collapse; width: 100%">
            ...
            <tr>
                ...
                <td style="text-align: center; border: 1px solid black">
                    {{ $devolucion->cargadores }}
                </td>
                <td style="text-align: center; border: 1px solid black">
                    {{ $devolucion->municiones }}
                </td>
            </tr>
            @endforeach
        </table>

        <p style="text-align: justify">Motivo de la devolución: {{$devolucion->obs}}</p>
