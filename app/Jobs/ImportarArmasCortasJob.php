...
use App\Imports\ExcelArmasCortas;
...
class ImportarArmasCortasJob implements ShouldQueue
{
    ...
    public function readExcel(string $tmpFileName, int $tipoArma, $fechaAlta, string $obs): bool
    {
        try {
            ...
            $import = new ExcelArmasCortas($tipoArma, $fechaAlta, $obs);
    ...
    protected function moveFileToProcessed($tmpFileName)
    {
        ...
        if ($newPath) {
            event(new ImportArmasStatusMessage('Fin del proceso de importación de armas cortas', $this->auth ));
        }
